...
import { Badge, Checkbox, Layout } from '@sonarsource/echoes-react';
...
export default function ApiSidebar({ apisList }: Readonly<Props>) {
  ...
  return (
    <Layout.AsideLeft size="large">
      <InputSearch
        className="sw-w-full"
        onChange={setSearch}
        placeholder={formatMessage({ id: 'api_documentation.v2.search' })}
        value={search}
      />
      ...
      <div className="sw-my-4">
        <Checkbox
          ...
          helpText={<FormattedMessage id="api_documentation.internal_tooltip" />}
          label={<FormattedMessage id="api_documentation.show_internal_v2" />}
          onCheck={() => {
      ...
      <nav aria-label={formatMessage({ id: 'api_documentation.page' })}>
