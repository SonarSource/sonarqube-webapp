...
import { getGitlabProjects } from '~sq-server-commons/api/alm-integrations';
...
import { mockGitlabProject } from '~sq-server-commons/helpers/mocks/alm-integrations';
...
it('should show import project feature when PAT is already set', async () => {
  ...
  expect(await ui.project1.find()).toBeInTheDocument();
  expect(ui.project1Link.get()).toHaveAttribute('href', '/project/overview?id=key');
