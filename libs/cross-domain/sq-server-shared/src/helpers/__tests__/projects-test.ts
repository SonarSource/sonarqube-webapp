...
describe('validateProjectKey', () => {
  it('should correctly flag an invalid key', () => {
    ...
    expect(validateProjectKey('foo/bar')).toBe(
      ProjectKeyValidationResult.InvalidChar,
    );
    // Cannot contain only numbers.
    expect(validateProjectKey('123')).toBe(
      ProjectKeyValidationResult.OnlyDigits,
    );
    // Cannot be more than 400 chars long.
    expect(
      validateProjectKey(new Array(PROJECT_KEY_MAX_LEN + 1).fill('a').join('')),
    ).toBe(ProjectKeyValidationResult.TooLong);
    // Cannot be empty.
    expect(validateProjectKey('')).toBe(ProjectKeyValidationResult.Empty);
  });

  it('should not flag a valid key', () => {
    expect(validateProjectKey('foo:bar_baz-12.is')).toBe(
      ProjectKeyValidationResult.Valid,
    );
