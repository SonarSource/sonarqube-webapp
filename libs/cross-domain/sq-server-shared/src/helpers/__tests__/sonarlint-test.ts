...
describe('open ide', () => {
  it('should send the correct request to the IDE to open an issue', async () => {
    ...
    branchName = 'branch-1';
    await testWith({
      branchLike: { name: branchName, isMain: false, excludedFromPurge: false },
    });
    ...
    await testWith({
      token: { token: tokenValue, name: tokenName } as NewUserToken,
    });
  });
});
...
describe('sendUserToken', () => {
  it('should send the token the right port', async () => {
    ...
    const result = await sendUserToken(SONARLINT_PORT_START, {
      ...token,
      isExpired: false,
    });
  ...
  it('should handle errors', async () => {
    ...
    const resp = new Response('Meh', {
      status: HttpStatus.BadRequest,
      statusText: 'I no likez',
    });
