...
export function createCondition(
  ...
  return postJSON('/api/qualitygates/create_condition', data).catch(
    throwGlobalError,
  );
}

export function updateCondition(data: Condition): Promise<Condition> {
  return postJSON('/api/qualitygates/update_condition', data).catch(
    throwGlobalError,
  );
}
...
export function getAllQualityGateProjects(
  ...
  return searchProjects({ ...data, pageSize: 1000 }).then((r) => {
    ...
    return getAllQualityGateProjects(
      { ...data, page: result.paging.pageIndex + 1 },
      result,
    );
  });
}
...
export function getApplicationQualityGate(data: {
  ...
  return getJSON('/api/qualitygates/application_status', data).catch(
    throwGlobalError,
  );
}
...
export function searchUsers(
  data: SearchPermissionsParameters,
): Promise<{ users: UserBase[] }> {
  return getJSON('/api/qualitygates/search_users', data).catch(
    throwGlobalError,
  );
}
...
export function searchGroups(
  ...
  return getJSON('/api/qualitygates/search_groups', data).catch(
    throwGlobalError,
  );
}
