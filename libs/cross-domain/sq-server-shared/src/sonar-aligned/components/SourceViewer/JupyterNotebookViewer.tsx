...
export function JupyterMarkdownCell({
  cell,
}: Readonly<{ cell: IMarkdownCell }>) {
...
function CellOutput({ output }: Readonly<{ output: IOutput }>) {
  if (isExecuteResult(output) || isDisplayData(output)) {
    const components = Object.entries(output.data).map(
      ([mimeType, dataValue], index) => {
    ...
    <div className={classNames('sw-m-4 sw-ml-0', className)}>
      ...
      <div>
        {outputs?.map((output: IOutput, outputIndex: number) => (
          <CellOutput
            key={`${output.output_type}-output-${outputIndex}`}
            output={output}
          />
        ))}
      </div>
