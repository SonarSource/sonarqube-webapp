...
import { IssueActions, IssueStatus, IssueType } from '../../../types/issues';
...
describe('updating', () => {
  it('should allow updating the status', async () => {
    ...
    renderIssue({
      issue: mockIssue(false, {
        ...pick(issue, 'key', 'status', 'transitions'),
      }),
    });
  ...
  it('should allow updating the tags', async () => {
    ...
    issuesHandler.setIssueList([{ issue, snippets: {} }]);
    renderIssue({
      issue: mockIssue(false, { ...pick(issue, 'actions', 'key', 'tags') }),
    });
...
it('should correctly handle keyboard shortcuts', async () => {
  ...
  renderIssue({
    ...
    selected: true,
    issue: mockIssue(false, {
      ...pick(issue, 'actions', 'key', 'assignee', 'transitions'),
    }),
  });
...
function getPageObject() {
  ...
  const selectors = {
    ...
    // Similar issues
    toggleSimilarIssuesBtn: byRole('button', {
    ...
    similarIssueProjectLink: byRole('button', {
      name: 'qualifier.TRK Project Bar',
    }),
    ...
    commentTextInput: byRole('textbox', {
      name: 'issue.comment.enter_comment',
    }),
