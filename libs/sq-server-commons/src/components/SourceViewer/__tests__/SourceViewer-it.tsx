...
it('should show SCM information', async () => {
  ...
  expect(
    secondRowScreen.queryByRole('cell', {
      name: 'stas.vilchik@sonarsource.com',
    }),
  ).not.toBeInTheDocument();

  // SCM with no date
  row = await screen.findByRole('row', {
    name: /\* mailto:info AT sonarsource DOT com$/,
  });
  expect(row).toBeInTheDocument();
...
it('should show duplication block', async () => {
  ...
  await user.click(
    duplicateLine.getByRole('button', {
      name: 'source_viewer.tooltip.duplicated_block',
    }),
  );
...
it('should show software quality measures in header', async () => {
  renderSourceViewer({
    componentMeasures: generateMeasures(),
    showMeasures: true,
  });
