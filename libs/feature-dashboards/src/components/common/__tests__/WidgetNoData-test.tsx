...
describe('WidgetNoData', () => {
  it('renders the default no-data message key', () => {
    renderWithRouter(<WidgetNoData />);

    expect(screen.getByText('dashboard.widget.no_data')).toBeInTheDocument();
  });

  it('renders a custom message key when provided', () => {
    renderWithRouter(<WidgetNoData messageKey="dashboard.widget.loading_visualization" />);
